...
def load_employee_csv_merged(path: str) -> List[Document]:
    ...
    st.caption(f"DEBUG 社員名簿CSV 読み込み成功: {os.path.basename(path)} / encoding={used_enc} / 総行数={len(rows)}")
    ...
    for dept, people in bucket.items():
        st.caption(f"DEBUG 部署: {dept} / {len(people)}名")
...
def initialize_retriever():
    ...
    st.caption(f"DEBUG 読み込んだドキュメント数: {len(docs_all)}")
    for i, d in enumerate(docs_all[:5]):
        st.caption(f"DEBUG doc[{i}].source = {d.metadata.get('source')}")
